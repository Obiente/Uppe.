...
impl Location {
    ...
    pub fn region_from_country(country_code: &str) -> &'static str {
        match country_code {
            ...
            // Asia
            "CN" | "JP" | "KR" | "IN" | "SG" | "HK" | "TW" | "TH" | "MY" | "ID" | "PH" | "VN" => "Asia",
            ...
            // Middle East
            "AE" | "SA" | "IL" | "TR" | "IR" | "IQ" | "JO" | "KW" | "QA" | "BH" | "OM" => "Middle East",
...
async fn fetch_location_from_ip() -> Result<Location> {
    ...
    let city = if !response.city.is_empty() {
        Some(response.city)
    } else {
        None
    };

    let country = if !response.country_code.is_empty() {
