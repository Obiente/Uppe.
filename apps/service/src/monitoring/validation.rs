...
fn validate_http_target(target: &str) -> Result<()> {
    ...
    if let Some(host) = url.host_str()
        && is_private_or_local(host)
    {
        return Err(anyhow!("Private/local addresses not allowed in public monitors: {}", host));
    }
...
fn validate_tcp_target(target: &str) -> Result<()> {
    ...
    if is_private_or_local(host) {
        return Err(anyhow!("Private/local addresses not allowed in public monitors"));
    }
...
fn is_private_or_local(host: &str) -> bool {
    ...
    if let Ok(ip) = host.parse::<IpAddr>() {
        ...
        matches!(
            host,
            "localhost" | "local" | "internal" | "private" | "*.local" | "*.internal" | "*.private"
        )
    }
}
...
fn validate_port_allowlist(port: u16) -> Result<()> {
    ...
    if port < 1024 {
        tracing::warn!("Monitor using privileged port {} - ensure this is intentional", port);
    }
