...
use peerup::distributed::{ConsensusManager, OrchestrationSchedule, PublicMonitorGroup};
...
impl DistributedOrchestrator {
    pub async fn get_peer_phase_offset(&self, domain: &str, peer_id: &str) -> u64 {
        let groups = self.groups.read().await;
        let Some(group) = groups.get(domain) else { return 0 };
        let mut peers = group.participating_peers.clone();
        peers.sort();
        peers.dedup();
        let Some(index) = peers.iter().position(|peer| peer == peer_id) else { return 0 };
        group.schedule.interval_seconds.saturating_mul(index as u64) / peers.len() as u64
    }
    pub async fn broadcast_all_for_discovery(&self) -> Result<()> {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        let public_monitors: Vec<_> = all_monitors.into_iter().filter(|m| m.is_public()).collect();
        ...
        for monitor in public_monitors {
            if let Some(domain) = &monitor.public_domain {
                domain_monitors.entry(domain.clone()).or_default().push(monitor);
            }
        }
    ...
    pub async fn handle_new_monitor(&self, monitor: &Monitor) -> Result<()> {
        ...
        if let Some(existing_group) = groups.get_mut(domain) {
            ...
            self.propose_schedule_update(domain, existing_group.schedule.clone()).await?;
        } else {
            // Create new group
            info!("Creating new public monitor group for {}", domain);

            let display_name =
                monitor.public_display_name.clone().unwrap_or_else(|| domain.clone());
            ...
            self.consensus.get_or_create(domain, group.schedule.clone()).await;
        }
    ...
    #[allow(dead_code, reason = "Coordination hooks retained for scheduler integration")]
    pub async fn should_check_now(&self, monitor: &Monitor) -> bool {
        ...
        self.consensus.should_check_now(domain, &self.peer_id).await
    }
    ...
    pub async fn mark_check_completed(&self, monitor: &Monitor) -> Result<()> {
        ...
        self.consensus.mark_check_completed(domain, &self.peer_id).await;
    ...
    async fn propose_schedule_update(
        ...
        let vote_data = format!("{}:{}:{}", domain, serde_json::to_string(&schedule)?, timestamp);
    ...
    pub async fn handle_vote(&self, vote: peerup::distributed::OrchestrationVote) -> Result<()> {
        {
            let groups = self.groups.read().await;
            let group = groups.get(&vote.domain).ok_or_else(|| anyhow::anyhow!("Unknown group"))?;
            if !group.participating_peers.contains(&vote.voter_peer_id)
                || vote.schedule.assignments.len() != group.participating_peers.len()
                || vote
                    .schedule
                    .assignments
                    .iter()
                    .any(|a| !group.participating_peers.contains(&a.peer_id))
            {
                anyhow::bail!("Vote does not match known group membership");
            }
            let distinct: std::collections::HashSet<_> =
                vote.schedule.assignments.iter().map(|a| &a.peer_id).collect();
            if distinct.len() != group.participating_peers.len() {
                anyhow::bail!("Duplicate schedule assignment");
            }
        }
        debug!("Received orchestration vote from {} for {}", vote.voter_peer_id, vote.domain);
        ...
        if let Some(group) = groups.get(&vote.domain) {
            ...
            if let Some(new_schedule) =
                self.consensus.check_consensus(&vote.domain, total_peers).await
            {
                info!("Consensus reached for {} - updating schedule", vote.domain);
    ...
    pub async fn handle_peer_join(&self, domain: String, peer_id: String) -> Result<()> {
        ...
        if let Some(group) = groups.get_mut(&domain) {
            ...
            warn!("Received join request for unknown monitor group: {}", domain);
        }
    ...
    async fn broadcast_vote(&self, vote: peerup::distributed::OrchestrationVote) -> Result<()> {
        let message =
            peerup::distributed::PublicMonitorMessage::Vote { vote: Box::new(vote.clone()) };
