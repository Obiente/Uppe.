...
type PendingAssignments = HashMap<String, (Vec<String>, SystemTime)>;
...
pub struct PrivateMonitorOrchestrator {
    ...
    pending_assignments: Arc<RwLock<PendingAssignments>>,
    ...
    uppe_service_peers: RwLock<HashSet<String>>,
...
impl PrivateMonitorOrchestrator {
    pub async fn get_assigned_monitor_uuids(&self) -> HashSet<uuid::Uuid> {
        let accepted = self.assignments.read().await;
    ...
    pub async fn update_uppe_service_peers(&self, peers: HashSet<String>) {
        *self.uppe_service_peers.write().await = peers;
    }
    /// Retry only monitors without accepted or in-flight assignments.
    pub async fn retry_unhelped_monitors(&self) -> Result<()> {
        for monitor in self.database.get_enabled_monitors().await? {
            let id = monitor.uuid.to_string();
            if monitor.is_private()
                && self.assignments.read().await.get(&id).is_none_or(Vec::is_empty)
                && !self.pending_assignments.read().await.contains_key(&id)
            {
                self.handle_new_monitor(&monitor).await?;
            }
        }
        Ok(())
    }
    ...
    pub fn new(
        ...
        Self {
            ...
            uppe_service_peers: RwLock::new(HashSet::new()),
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        let mut private_monitors: Vec<_> =
            all_monitors.into_iter().filter(|m| m.is_private()).collect();
                ...
                Ok(helpers) => {
                    if !helpers.is_empty() {
                        ...
                        self.assignments
                            .write()
                            .await
                            .insert(monitor.uuid.to_string(), helpers.clone());
    ...
    #[allow(dead_code, reason = "Experimental peer recovery interface; not enabled by default")]
    ...
    pub async fn handle_new_monitor(&self, monitor: &Monitor) -> Result<()> {
        ...
        if helpers.is_empty() {
            warn!("No helper peers available for private monitor {}", monitor_uuid);
            return Ok(());
        }
    ...
    async fn assign_helper_peers(&self, monitor: &Monitor) -> Result<Vec<String>> {
        ...
        let service_peers = self.uppe_service_peers.read().await;
        ...
        let status = self.helper_status.read().await;
        let mut candidates: Vec<String> = connected
            .iter()
            .filter(|peer_id| {
                *peer_id != owner_id
                    && service_peers.contains(*peer_id)
                    && status
                        .get(*peer_id)
                        .is_none_or(|entry| entry.assignment_count < MAX_ASSIGNMENTS_PER_HELPER)
            })
            .cloned()
            .collect();
    ...
    async fn notify_helper_peer(&self, helper_id: &str, monitor: &Monitor) -> Result<()> {
        ...
        let request = crate::p2p::messages::HelperAssignmentRequest {
            ...
            owner_libp2p_peer_id: peerup::crypto::transport_peer_id(&hex::decode(&self.peer_id)?)?,
            check_offset_seconds: 0,
        };
    ...
    async fn gossip_encrypted_result(&self, result: &EncryptedResult) -> Result<()> {
        debug!("Gossiping encrypted result for owner {}", result.owner_peer_id);

        // Publish via P2P network to private results topic
        self.p2p_network
            .send_command(crate::p2p::messages::P2PCommand::PublishEncryptedResult(result.clone()))
            .await?;
    ...
    async fn store_in_dht_batch(&self, result: &EncryptedResult) -> Result<()> {
        ...
        let key = format!(
            "/uppe/private/{}/{}/{}",
            result.owner_peer_id, result.monitor_uuid, result.encrypted_at
        );
    ...
    pub async fn sync_owner_results_from_dht(&self, _owner_secret_key: &[u8; 32]) -> Result<()> {
        anyhow::bail!(
            "Offline result recovery requires a resumable authenticated transfer protocol; it is not implemented"
        )
    }

    /// Handle helper assignment acceptance
    pub async fn handle_helper_accepted(
        &self,
        monitor_uuid: &str,
        helper_peer_id: &str,
    ) -> Result<()> {
        let mut pending = self.pending_assignments.write().await;
        let Some((helpers, _)) = pending.get_mut(monitor_uuid) else {
            anyhow::bail!("No pending assignment");
        };
        let Some(index) = helpers.iter().position(|id| id == helper_peer_id) else {
            anyhow::bail!("Response from an unassigned helper");
        };
        ...
        if let Some(status) = status_map.get_mut(helper_peer_id) {
            ...
            status_map.insert(
                ...
                HelperStatus {
                    _peer_id: helper_peer_id.to_string(),
        ...
        // Keep other requested helpers pending until each responds.
        {
            let mut assignments = self.assignments.write().await;
            assignments
    ...
    pub async fn is_assigned_helper(&self, monitor_uuid: &str, peer: &str) -> bool {
        self.assignments
            .read()
            .await
            .get(monitor_uuid)
            .is_some_and(|ids| ids.iter().any(|id| id == peer))
    }

    /// Handle helper assignment rejection
    pub async fn handle_helper_rejected(
        &self,
        monitor_uuid: &str,
        helper_peer_id: &str,
        reason: &str,
        ...
        warn!(
            "Helper {} rejected assignment for monitor {}: {}",
            helper_peer_id, monitor_uuid, reason
        );
        ...
        if let Ok(Some(monitor)) =
            self.database.get_monitor_by_uuid(uuid::Uuid::parse_str(monitor_uuid)?).await
            && monitor.is_private()
        {
            info!("Finding replacement helper for monitor {}", monitor_uuid);
            if let Err(e) = self.handle_new_monitor(&monitor).await {
            ...
            for (monitor_uuid, helpers) in assignments {
                ...
                if has_stale {
                    ...
                    if let Ok(Some(monitor)) = self
                        .database
                        .get_monitor_by_uuid(uuid::Uuid::parse_str(&monitor_uuid)?)
                        .await
                    {
...
struct OwnerLimits {
    ...
    #[allow(dead_code, reason = "Used by the experimental assignment budget")]
