...
async fn test_retention_policy_integration() -> Result<()> {
    ...
    assert!(database.get_peer_results(old_result.monitor_uuid, 10).await?.is_empty());
...
async fn test_owner_sync_with_empty_dht() -> Result<()> {
    ...
    assert!(orchestrator.sync_owner_results_from_dht(&owner_secret_key).await.is_err());
