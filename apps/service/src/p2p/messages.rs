...
pub struct HelperAssignmentRequest {
    ...
    pub assigned_at: i64,
    #[serde(default)]
    pub owner_libp2p_peer_id: String,
    #[serde(default)]
    pub check_offset_seconds: u64,
}
...
pub enum HelperAssignmentResponse {
    ...
    Accepted { monitor_uuid: String, helper_peer_id: String, owner_libp2p_peer_id: String },
    /// Helper rejected (rate limited, too many assignments, etc.)
    Rejected { monitor_uuid: String, reason: String, owner_libp2p_peer_id: String },
}

/// Commands sent to the P2P node
#[derive(Debug, Clone)]
#[allow(dead_code, reason = "Wire command variants reserved for peer recovery")]
pub enum P2PCommand {
    AnnouncePresence,
    DialBootstrapPeers,
...
#[derive(Debug, Clone)]
#[allow(dead_code, reason = "Wire event variants reserved for peer discovery and recovery")]
pub enum P2PEvent {
    NodeAnnounced {
        libp2p_peer_id: String,
        app_peer_id: String,
    },
    PublicMonitorAnnounced {
        domain: String,
        display_name: String,
    ...
    PublicMonitorGroupMessage {
        _from_peer: String,
        domain: String,
        message: Box<peerup::distributed::PublicMonitorMessage>,
    },

    /// A monitoring result was received from a peer
    ResultReceived {
        peer_id: String,
        result: Box<PeerResult>,
    },
    /// Results retrieved from a peer query
    ResultsQueried {
        peer_id: String,
        results: Box<ResultsQueryResponse>,
    },
    /// Peer notified us they synced our data
    SyncCompleted {
        peer_id: String,
        notification: Box<SyncCompletionNotification>,
    },
