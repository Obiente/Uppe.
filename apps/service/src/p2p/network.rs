...
impl P2PNetwork {
    ...
    pub async fn start(&mut self) -> anyhow::Result<mpsc::Receiver<P2PEvent>> {
        ...
        node.subscribe_to_topic("/uppe/announce/v1")?;
            ...
            loop {
                tokio::select! {
                    ...
                    Some(cmd) = command_rx.recv() => {
                        match cmd {
                            P2PCommand::AnnouncePresence => {
                                if let Some(key) = public_key
                                    && let Ok(bytes) = serde_json::to_vec(&key.to_vec()) {
                                        let _ = node.publish_to_topic("/uppe/announce/v1", bytes);
                                    }
                            }
                            P2PCommand::DialBootstrapPeers => {
                                let _ = node.dial_bootstrap_peers(&bootstrap_peers);
                            }
                    ...
                    event = node.swarm.select_next_some() => {
                        ...
                        match event {
                            SwarmEvent::Behaviour(PeerUPEvent::GossipsubMessage { peer: _, message, .. }) => {
                                let Some(peer)=message.source else {continue};
                                if message.data.len()>65_536 {continue;}
                                if ingress_window.elapsed() >= std::time::Duration::from_secs(1) {
                                    ingress_window=std::time::Instant::now(); ingress_count=0; author_counts.clear();
                                }
                                // Bound CPU, storage writes and author bookkeeping, including Sybil churn.
                                if ingress_count >= 20 {continue;}
                                ingress_count += 1;
                                let count=author_counts.entry(peer.to_string()).or_default();
                                if *count >= 5 {continue;}
                                *count+=1;
                                if message.topic.to_string()=="/uppe/announce/v1" {
                                ...
                                if topic_str.starts_with("/uppe/public-monitors/") {
                                    if let Ok(group_message) = serde_json::from_slice::<peerup::distributed::PublicMonitorMessage>(&message.data) {
                                ...
                                // Signed GossipSub binds responses to their original author, not a relay.
                                if topic_str=="/uppe/helper-assignments/v1"
                                    && let Ok(response)=serde_json::from_slice::<super::messages::HelperAssignmentResponse>(&message.data) {
                                        use super::messages::HelperAssignmentResponse;
                                        let owner=match &response {HelperAssignmentResponse::Accepted{owner_libp2p_peer_id,..}|HelperAssignmentResponse::Rejected{owner_libp2p_peer_id,..}=>owner_libp2p_peer_id};
                                        if owner==&libp2p_peer_id_str {
                                            let _=event_tx.send(P2PEvent::HelperAssignmentResponse{from_peer:peer.to_string(),response:Box::new(response)}).await;
                                        }
                                        continue;
                                    }
                                ...
                                if topic_str.starts_with("/uppe/private-results/")
                                    && let Ok(msg_str) = String::from_utf8(message.data.clone())
                                        && let Ok(encrypted_result) = serde_json::from_str::<crate::crypto::EncryptedResult>(&msg_str) {
    ...
    #[allow(dead_code, reason = "Reserved for resumable history recovery")]
    pub async fn get_dht_record(&self, key: &str) -> anyhow::Result<Option<Vec<u8>>> {
        ...
        if let Some(tx) = &self.command_tx {
            ...
            self.pending_dht_queries.write().await.insert(key.to_string(), response_tx);

            // Send the DHT query command
            tx.send(P2PCommand::GetDHTRecord { key: key.as_bytes().to_vec() })
                .await
    ...
    #[allow(dead_code, reason = "DHT publication adapter used by experimental recovery")]
    pub async fn put_dht_record(&self, key: &str, value: Vec<u8>) -> anyhow::Result<()> {
        ...
        if let Some(tx) = &self.command_tx {
            tx.send(P2PCommand::PublishDHTRecord { key: key.as_bytes().to_vec(), value })
                .await
    ...
    pub async fn publish_encrypted_result(
        &self,
        encrypted_result: &crate::crypto::EncryptedResult,
    ) -> anyhow::Result<()> {
        if !self.enabled {
            return Ok(());
        }
        ...
        if let Some(tx) = &self.command_tx {
            tx.send(P2PCommand::PublishEncryptedResult(encrypted_result.clone()))
                .await
                .map_err(|e| {
                    anyhow::anyhow!("Failed to send encrypted result publish command: {}", e)
                })?;
            tracing::debug!(
                "Sent publish encrypted result command for monitor {}",
                encrypted_result.monitor_uuid
            );
        } else {
