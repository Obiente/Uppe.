...
pub fn subscribe() -> broadcast::Receiver<TuiEvent> {
    bus().subscribe()
}

pub fn publish_peers(peers: Vec<String>, now: SystemTime) {
    let peer_models: Vec<Peer> = peers.into_iter().map(|id| Peer::new_online(id, now)).collect();
    debug!(count = peer_models.len(), "TUI bus: publishing peers update");
    publish(TuiEvent::Peers(peer_models));
}

pub fn publish_network_stats(stats: NetworkStats) {
    publish(TuiEvent::NetworkStats(stats));
}
...
static BACKEND_RUNNING: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
pub fn set_backend_running(running: bool) {
    BACKEND_RUNNING.store(running, std::sync::atomic::Ordering::Relaxed);
}
pub fn backend_running() -> bool {
    BACKEND_RUNNING.load(std::sync::atomic::Ordering::Relaxed)
}
