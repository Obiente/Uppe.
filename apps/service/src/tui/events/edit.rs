        ...
        match state.edit_field_index {
            ...
            2 => {
                if let Some(domain) = m.public_domain.as_mut()
                    && state.text_cursor < domain.len()
                {
                    domain.remove(state.text_cursor);
                }
            }
            3 => {
                if let Some(dn) = m.public_display_name.as_mut()
    ...
    match key_code {
        ...
        KeyCode::BackTab | KeyCode::Up | KeyCode::Char('k') => {
            state.edit_field_index =
                state.edit_field_index.checked_sub(1).unwrap_or(layout.total - 1);
            state.text_cursor = 0;
            state.update_text_cursor();
            state.validation_error = None;
        }
                    ...
                    && let Some(m) = state.edit_monitor.take()
                {
                    match db.save_monitor(&m).await {
                        Ok(_) => {
                            ...
                            state.set_status(
                                "Monitor saved",
                                crate::tui::state::StatusLevel::Success,
                            );
                        }
                        Err(e) => {
                ...
                match ch {
                    'c' => {
                        if state.edit_field_index == layout.check_type
                            && let Some(m) = state.edit_monitor.as_mut()
                        {
