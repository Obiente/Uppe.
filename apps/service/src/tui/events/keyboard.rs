...
pub async fn handle_main_view(
    ...
    match key.code {
        ...
        KeyCode::Char('h') | KeyCode::Left
            if key.modifiers.is_empty() && state.view_mode == ViewMode::Dashboard =>
        {
            state.focus = Focus::Monitors;
        }
        ...
        KeyCode::Char('x') if key.modifiers.is_empty() && state.view_mode == ViewMode::DhtDebug => {
            if state.dht_snapshot.is_none() || !crate::tui::bus::backend_running() {
                state.set_status(
                    "Live DHT queries are unavailable in this session",
                    crate::tui::state::StatusLevel::Error,
                );
            } else {
                ...
                if let Some(key) = key {
                    ...
                    {
                        crate::tui::bus::publish_dht_query(key.clone());
                    }
                    state.set_status(
                        format!("DHT query: {}", key),
                        crate::tui::state::StatusLevel::Info,
                    );
                } else {
        ...
        KeyCode::Char('A')
            if state.view_mode == ViewMode::Dashboard && state.focus == Focus::Monitors =>
        {
            let mut m =
                Monitor::new_public("".into(), "".into(), "".into(), "".into(), "http".into());
            m.interval_seconds = 30;
            m.timeout_seconds = 10;
            state.edit_monitor = Some(m);
            state.show_edit = true;
        ...
        KeyCode::Char('d')
            if key.modifiers.is_empty()
                && state.view_mode == ViewMode::Dashboard
                && state.focus == Focus::Monitors
                && state.monitors.get(state.selected).is_some() =>
        {
...
fn dht_key_for_cursor(state: &AppState) -> Option<String> {
    ...
    if !state.consensus_states.is_empty() {
        ...
        for domain in state.consensus_states.keys() {
            if row_idx == cursor {
...
mod regression_tests {
    use super::*;
    #[tokio::test]
    async fn dashboard_tab_and_distributed_navigation_use_the_visible_pane() {
        let dir = tempfile::tempdir().unwrap();
        let db = libsql::Builder::new_local(dir.path().join("synthetic.db"))
            .build()
            .await
            .unwrap();
        let pool = deadpool::managed::Pool::builder(crate::pool::LibsqlManager::new(db))
            .build()
            .unwrap();
        let database = DatabaseImpl::new_from_pool(pool);
        let mut state = AppState::new();
        handle_main_view(&mut state, KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE), &database)
            .await
            .unwrap();
        assert_eq!(state.view_mode, ViewMode::Dashboard);
        assert!(state.focus == Focus::Results);
        handle_main_view(
            &mut state,
        ...
        .await
        .unwrap();
        state.public_monitors = vec![
            Monitor::new("First".into(), "https://example.com".into(), "http".into()),
            Monitor::new("Second".into(), "https://example.net".into(), "http".into()),
        ];
        handle_main_view(
            &mut state,
        ...
        .await
        .unwrap();
        assert_eq!(state.selected_public_monitor, 1);
