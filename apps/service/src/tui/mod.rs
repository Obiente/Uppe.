    ...
    loop {
        ...
        while let Ok(ev) = bus_rx.try_recv() {
            match ev {
                ...
                bus::TuiEvent::DhtQueryResult { key, ok, bytes: _ } => {
                    ...
                    if ok {
                        state.dht_successful_queries += 1;
                    } else {
                        state.dht_failed_queries += 1;
                    }
            ...
            && !state.any_popup_open()
        {
            ...
            if state.peers.is_empty()
                && let Ok(peers) = db.list_peers(200).await
            {
                state.peers = peers;
            }
...
// Restore the terminal on normal return, propagated errors, and unwinding.
struct TerminalRestore;
impl Drop for TerminalRestore {
    fn drop(&mut self) {
        let _ = disable_raw_mode();
        let _ = execute!(std::io::stdout(), DisableMouseCapture, Show, LeaveAlternateScreen);
    }
}
