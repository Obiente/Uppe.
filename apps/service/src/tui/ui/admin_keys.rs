...
fn render_status(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        let stats_lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if stats.valid_keys > 0 {
                        Color::Green
                    } else {
                        Color::Red
                    }),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if stats.expired_keys > 0 {
                        Color::Yellow
                    } else {
                        Color::Gray
                    }),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if stats.revoked_keys > 0 {
                        Color::Red
                    } else {
                        Color::Gray
                    }),
    ...
    let info_lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "All keys are verified cryptographically before acceptance.",
...
fn render_key_list(f: &mut Frame, area: Rect, state: &AppState) {
    if state.admin_key_stats.is_none() {
        let empty = Paragraph::new(vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Admin keys are fetched from the bootstrap server",
                Style::default().fg(Color::Gray),
            )),
            Line::from(Span::styled(
                "when the backend orchestrator starts.",
                Style::default().fg(Color::Gray),
            )),
        ])
...
fn render_rotation_history(f: &mut Frame, area: Rect, _state: &AppState) {
    let empty = Paragraph::new(vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "Key rotations will appear here when admin keys are rotated.",
            Style::default().fg(Color::Gray),
        )),
    ])
