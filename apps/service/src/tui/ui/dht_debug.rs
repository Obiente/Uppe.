...
use super::{
    COLOR_ACTIVE, COLOR_BRAND, COLOR_ERROR, COLOR_INFO, COLOR_LABEL, COLOR_MUTED, COLOR_SUCCESS,
};
    ...
    if let Some(snapshot) = &state.dht_snapshot {
        for (b_idx, bucket) in snapshot.buckets.iter().enumerate() {
            ...
            for (p_idx, p) in bucket.peers.iter().enumerate() {
                let addr =
                    if p.addrs.is_empty() { "(no addrs)".to_string() } else { p.addrs[0].clone() };
...
fn render_dht_table(
    ...
    let styled_rows: Vec<Row> = row_data
        .iter()
        .enumerate()
        .map(|(i, (row, meta))| {
    ...
    if let Some((_, meta)) = row_data.get(state.dht_cursor) {
        match meta {
            DhtRow::BucketHeader { bucket_idx, peer_count } => {
                if let Some(snapshot) = &state.dht_snapshot
                    && let Some(bucket) = snapshot.buckets.get(*bucket_idx)
                {
            ...
            DhtRow::DhtPeer { bucket_idx, peer_idx } => {
                if let Some(snapshot) = &state.dht_snapshot
                    && let Some(bucket) = snapshot.buckets.get(*bucket_idx)
                    && let Some(peer) = bucket.peers.get(*peer_idx)
                {
                    lines.push(Line::from(vec![
...
fn render_footer(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let success_rate = state
        ...
        .unwrap_or(0);
    let success_color = if total_queries == 0 {
        COLOR_MUTED
    ...
    footer_lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{} ok  ", state.dht_successful_queries),
            Style::default().fg(COLOR_SUCCESS),
        ),
        Span::styled(
            format!("{} fail  ", state.dht_failed_queries),
            Style::default().fg(COLOR_ERROR),
        ),
        Span::styled(
            format!("{} pending  ", state.dht_pending_queries),
...
fn truncate_peer_id(id: &str) -> String {
    if id.len() > 20 { format!("{}..{}", &id[..8], &id[id.len() - 8..]) } else { id.to_string() }
}
