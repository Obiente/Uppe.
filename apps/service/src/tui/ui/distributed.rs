...
fn render_public_monitors(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|(i, monitor)| {
            ...
            let display_name = monitor.public_display_name.as_deref().unwrap_or(domain);
            ...
            let status_color = if monitor.enabled { Color::Green } else { Color::Gray };
            ...
            let content = Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(format!("({})", check_type), Style::default().fg(Color::Cyan)),
                Span::raw(" "),
                Span::styled(format!("{}s", interval), Style::default().fg(Color::DarkGray)),
            ]);
    ...
    if items.is_empty() {
        let empty = Paragraph::new(vec![
            ...
            Line::from(Span::styled(
                "Press Shift+A in Dashboard to add one",
...
fn render_public_monitor_detail(
    ...
    let info = Paragraph::new(info_text)
        .block(Block::default().borders(Borders::ALL).title("Monitor Details"));

    f.render_widget(info, chunks[0]);

    // Peer coordination info
    if let Some(group) = state.public_monitor_groups.iter().find(|g| g.domain == domain) {
        let peer_info = vec![
            Line::from(vec![
        ...
        let coordination = Paragraph::new(peer_info)
            .block(Block::default().borders(Borders::ALL).title("Coordination Status"));
...
fn render_pending_promotion(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let rows: Vec<Row> = state
        ...
        .map(|(domain, consensus)| {
            ...
            let status_color = if is_promoted { Color::Green } else { Color::Red };
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title("Pending Promotion (Threshold: 5 peers)"),
    )
    ...
    .block(Block::default().borders(Borders::ALL).title("Consensus Info"));
...
fn render_peer_groups(f: &mut Frame, area: Rect, state: &AppState) {
    let items: Vec<ListItem> = state
        ...
        .map(|group| {
            ...
            let content = Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(format!("({} peers)", peer_count), Style::default().fg(Color::Green)),
                Span::raw(" "),
                Span::styled(format!("{}s interval", interval), Style::default().fg(Color::Cyan)),
                Span::raw(" "),
                Span::styled(format!("{} checks", checks), Style::default().fg(Color::DarkGray)),
            ]);
    ...
    if items.is_empty() {
        let empty = Paragraph::new(vec![
            Line::from(""),
            Line::from(Span::styled(
                "No peer groups formed yet",
                Style::default().fg(Color::DarkGray),
            )),
            Line::from(Span::styled(
                "Groups form when peers coordinate on public monitors",
                Style::default().fg(Color::Gray),
            )),
        ])
...
fn render_rate_limits(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let config = vec![
        Line::from(vec![
            Span::styled("Minimum Check Interval: ", Style::default().fg(Color::Cyan)),
            Span::raw("10 seconds"),
        ]),
        Line::from(vec![
            Span::styled("Maximum Checks/Hour/Peer: ", Style::default().fg(Color::Cyan)),
    ...
    let config_widget = Paragraph::new(config)
        .block(Block::default().borders(Borders::ALL).title("Rate Limit Configuration"));
    ...
    if !state.rate_limit_stats.is_empty() {
        ...
        let rows: Vec<Row> = state
            ...
            .map(|(peer_id, count)| {
                ...
                let status_color = if *count < 360 { Color::Green } else { Color::Red };
        ...
        .header(header)
        .block(Block::default().borders(Borders::ALL).title("Per-Peer Rate Limits"));
...
fn render_distributed_summary(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let private_count = state.monitors.len().saturating_sub(public_count);
