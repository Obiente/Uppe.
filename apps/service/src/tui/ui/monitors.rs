...
pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
    let items: Vec<ListItem> = state.monitors.iter().enumerate().map(|(i, m)| {
    ...
    let monitors_title = if state.focus == Focus::Monitors {
        "Monitors (focused)"
    } else {
        "Monitors"
    };
    ...
    let monitors_list = List::new(items)
        .block(Block::default().borders(Borders::ALL).title(monitors_title));
