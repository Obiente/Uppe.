...
pub fn render(f: &mut Frame, size: Rect, state: &AppState) {
    ...
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("Enter to query, Esc to cancel", Style::default().fg(Color::Gray))),
    ];
    ...
    let popup =
        Paragraph::new(lines).block(Block::default().borders(Borders::ALL).title("DHT GET"));
