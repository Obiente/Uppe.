...
pub fn render(f: &mut Frame, size: Rect, state: &AppState) {
    if let Some(m) = &state.edit_monitor {
        ...
        let title = if state.is_add_form {
            "Add Monitor"
        } else {
            "Edit Monitor"
        };
        ...
        lines.push(Line::from("  Tab/Shift-Tab/j/k: Next/Prev field  Left/Right/h/l: Move cursor/adjust"));
        lines.push(Line::from("  Home/End: Jump to start/end of text"));
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled("Edit:", Style::default().fg(Color::Gray))));
        lines.push(Line::from("  Type: edit text  +/- [ ]: adjust numbers  C: cycle type  Space: toggle"));
