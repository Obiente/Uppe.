...
pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let border_style =
        if focused { Style::default().fg(COLOR_ACTIVE) } else { Style::default().fg(COLOR_BRAND) };
    ...
    if !state.monitors.is_empty() && state.selected < state.monitors.len() {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                format!("{:.1}%", stats.uptime),
                Style::default().fg(color).add_modifier(Modifier::BOLD),
            ),
            Span::styled(
                format!("  ({}/{})", stats.successful, stats.total),
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!(
                        ...
                        stats.min_latency, stats.max_latency, stats.p95_latency
                    ),
                    Style::default().fg(COLOR_MUTED),
                ),
            ]));
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                format!("  timeout {}s", monitor.timeout_seconds),
                Style::default().fg(COLOR_MUTED),
            ),
        ]));
    } else {
