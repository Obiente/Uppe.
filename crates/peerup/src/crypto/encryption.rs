...
//!
//! # Security Model
//! - Ephemeral-static Diffie-Hellman with HKDF key derivation
//! - Owner key compromise can expose previously recorded ciphertext
...
fn derive_key(shared: &x25519_dalek::SharedSecret, context: &[u8]) -> Result<XChaCha20Poly1305> {
    if !shared.was_contributory() {
        return Err(anyhow!("Invalid X25519 public key"));
    }
    let mut key = [0u8; 32];
    hkdf::Hkdf::<sha2::Sha256>::new(Some(b"uppe/encrypted-result/v2"), shared.as_bytes())
        .expand(context, &mut key)
        .map_err(|_| anyhow!("Key derivation failed"))?;
    Ok(XChaCha20Poly1305::new((&key).into()))
}

fn metadata(owner: &str, monitor: &str, helper: &str, timestamp: i64) -> Result<Vec<u8>> {
    Ok(serde_json::to_vec(&(2u8, owner, monitor, helper, timestamp))?)
}
...
pub fn encrypt_result_for_owner<T: Serialize>(
    ...
    let encrypted_at = chrono::Utc::now().timestamp();
    let context = metadata(&owner_peer_id, &monitor_uuid, &helper_peer_id, encrypted_at)?;
    let key = derive_key(&shared_secret, &context)?;
...
pub fn decrypt_result_for_owner<T: for<'de> Deserialize<'de>>(
    ...
    if encrypted.version != 2 {
        return Err(anyhow!("Unsupported encrypted result version"));
    }
    ...
    let context = metadata(
        &encrypted.owner_peer_id,
        &encrypted.monitor_uuid,
        &encrypted.helper_peer_id,
        encrypted.encrypted_at,
    )?;
    let key = derive_key(&shared_secret, &context)?;
    ...
    let plaintext = key
        .decrypt(&nonce, Payload { msg: &encrypted.ciphertext, aad: &context })
...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        let data = TestData { message: "test encrypted data".to_string(), value: 42 };
    ...
    fn test_decrypt_with_wrong_key_fails() {
        ...
        let data = TestData { message: "secret".to_string(), value: 99 };
    ...
    fn test_keypair_integration() {
        ...
        let data = TestData { message: "keypair integration test".to_string(), value: 7 };
    ...
    fn test_forward_secrecy() {
        ...
        let data = TestData { message: "same data".to_string(), value: 1 };
        ...
        let dec1: TestData = decrypt_result_for_owner(&enc1, &owner_secret.to_bytes()).unwrap();
        let dec2: TestData = decrypt_result_for_owner(&enc2, &owner_secret.to_bytes()).unwrap();
        assert_eq!(dec1, dec2);
    }
    ...
    fn test_batch_operations() {
        ...
        for i in 0..5u32 {
            let data = TestData { message: format!("batch item {}", i), value: i };
            let encrypted = encrypt_result_for_owner(
                &data,
                &owner_public.to_bytes(),
                format!("helper-{}", i),
