...
impl KeyPair {
    ...
    pub fn new(signing_key: SigningKey) -> Self {
        ...
        let x25519_public = X25519PublicKey::from(&X25519StaticSecret::from(x25519_secret_bytes));
...
pub fn save_keypair(keypair: &KeyPair, path: &Path) -> Result<()> {
    ...
    let mut file =
        options.open(path).context("Failed to create keypair file without overwriting")?;
