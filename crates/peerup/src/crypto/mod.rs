...
/// Bind an application Ed25519 key to its authenticated transport identity.
pub fn transport_peer_id(public_key: &[u8]) -> anyhow::Result<String> {
    let key = libp2p::identity::ed25519::PublicKey::try_from_bytes(public_key)?;
    let key = libp2p::identity::PublicKey::from(key);
    Ok(libp2p::PeerId::from_public_key(&key).to_string())
}
