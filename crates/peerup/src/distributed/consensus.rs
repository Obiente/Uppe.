...
pub struct ConsensusState {
    ...
    last_vote_timestamps: HashMap<String, i64>,
...
impl RateLimitState {
    ...
    pub fn validate_schedule(
        ...
        let divisor = schedule
            .interval_seconds
            .checked_mul(peer_count as u64)
            .ok_or("Schedule interval overflow")?;
        if divisor == 0 {
            return Err("Invalid schedule: interval cannot be zero".to_string());
        }
        let checks_per_peer_per_hour = 3600_u64.div_ceil(divisor);
...
impl ConsensusState {
    ...
    pub fn new(domain: String, initial_schedule: super::visibility::OrchestrationSchedule) -> Self {
        Self {
            ...
            last_vote_timestamps: HashMap::new(),
    ...
    pub fn cast_vote(&mut self, vote: OrchestrationVote) -> Result<(), String> {
        ...
        self.pending_votes.retain(|v| v.timestamp >= now - 300);
        self.last_vote_timestamps.retain(|_, timestamp| *timestamp >= now - 300);
        if self
            .last_vote_timestamps
            .get(&vote.voter_peer_id)
            .is_some_and(|timestamp| vote.timestamp <= *timestamp)
        {
            return Err("Replayed vote".into());
        }
        if vote.domain != self.domain || vote.timestamp > now + 30 || vote.timestamp < now - 300 {
            return Err("Vote domain or timestamp is invalid".into());
        }
        if self.pending_votes.len() >= 1024 {
            return Err("Too many pending votes".into());
        }
        // Validate signature
        ...
        self.rate_limit.validate_schedule(&vote.schedule, vote.schedule.assignments.len())?;
        ...
        self.last_vote_timestamps.insert(vote.voter_peer_id.clone(), vote.timestamp);
    ...
    pub fn check_consensus(
        &mut self,
        total_peers: usize,
    ) -> Option<super::visibility::OrchestrationSchedule> {
        let now = chrono::Utc::now().timestamp();
        self.pending_votes.retain(|vote| vote.timestamp >= now - 300 && vote.timestamp <= now + 30);
        if total_peers == 0 || self.pending_votes.is_empty() {
            return None;
        }
        ...
        let (winning_schedule_json, winning_votes) =
            vote_groups.iter().max_by_key(|(_, votes)| votes.len())?;
    ...
    fn verify_vote_signature(&self, vote: &OrchestrationVote) -> bool {
        ...
        if vote.voter_peer_id != hex::encode(public_key_bytes) {
            return false;
        }
    ...
    pub fn should_check_now(&mut self, peer_id: &str) -> bool {
        ...
        if let Some(assignment) =
            self.current_schedule.assignments.iter().find(|a| a.peer_id == peer_id)
        {
            let now = chrono::Utc::now().timestamp();
    ...
    pub fn mark_check_completed(&mut self, peer_id: &str) {
        ...
        let assignment_idx =
            self.current_schedule.assignments.iter().position(|a| a.peer_id == peer_id);
...
impl ConsensusManager {
    pub fn new() -> Self {
        Self { states: tokio::sync::RwLock::new(HashMap::new()) }
    }
    ...
    async fn test_consensus() {
        ...
        for _ in 0..3 {
            ...
            let vote = OrchestrationVote {
                ...
                voter_peer_id: keypair.public_key_hex(),
