...
impl PublicMonitorDHT {
    pub fn new() -> Self {
        Self { local_cache: tokio::sync::RwLock::new(HashMap::new()) }
    }
    ...
    pub async fn list_all(&self) -> Vec<PublicMonitorGroup> {
        self.local_cache.read().await.values().cloned().collect()
    }
...
mod tests {
    ...
    async fn test_publish_and_lookup() {
        ...
        dht.publish_monitor("google.com".to_string(), group.clone()).await.unwrap();
    ...
    async fn test_join_leave() {
        ...
        dht.publish_monitor("github.com".to_string(), group).await.unwrap();

        dht.join_monitor("github.com", "peer2".to_string()).await.unwrap();
