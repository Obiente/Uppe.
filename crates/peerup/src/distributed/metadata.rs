...
impl PeerMetadataDHT {
    ...
    pub async fn prepare_rate_limits(
        ...
        let limits =
            cache.get(peer_id).cloned().unwrap_or_else(|| PeerRateLimits::new(peer_id.to_string()));
        drop(cache);
    ...
    pub async fn process_rate_limits_record(
        ...
        // Update cache
        self.rate_limits_cache.write().await.insert(limits.peer_id.clone(), limits.clone());
    ...
    pub async fn update_rate_limits(&self, limits: PeerRateLimits) {
        self.rate_limits_cache.write().await.insert(limits.peer_id.clone(), limits);
    }
    ...
    pub async fn prepare_trust_score(
        ...
        let score =
            cache.get(peer_id).cloned().unwrap_or_else(|| PeerTrustScore::new(peer_id.to_string()));
        drop(cache);
    ...
    pub async fn process_trust_score_record(
        ...
        // Update cache
        self.trust_scores_cache.write().await.insert(score.peer_id.clone(), score.clone());
    ...
    pub async fn update_trust_score(&self, score: PeerTrustScore) {
        self.trust_scores_cache.write().await.insert(score.peer_id.clone(), score);
    }
