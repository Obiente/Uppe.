...
impl PublicMonitorGroup {
    ...
    pub fn next_checker(&self) -> Option<&str> {
        self.schedule.assignments.iter().min_by_key(|a| a.next_check_at).map(|a| a.peer_id.as_str())
    }

    /// Update check time for a peer after they complete a check
    pub fn mark_check_completed(&mut self, peer_id: &str) {
        if let Some(assignment) =
            self.schedule.assignments.iter_mut().find(|a| a.peer_id == peer_id)
        {
            assignment.next_check_at +=
                (self.schedule.interval_seconds * self.participating_peers.len() as u64) as i64;
            self.total_checks += 1;
        }
    }
}
...
mod tests {
    ...
    fn test_add_peer_rebalances() {
        ...
        let times: Vec<i64> = group.schedule.assignments.iter().map(|a| a.next_check_at).collect();
    ...
    fn test_retention_policy() {
        ...
        let private = MonitorVisibility::Private { owner_peer_id: "peer1".to_string() };
