...
pub enum PeerUPEvent {
    ...
    DhtGetRecordOk { key: Vec<u8>, record: Vec<u8> },
    /// DHT get record not found
    DhtGetRecordErr { key: Vec<u8> },
    /// DHT put record success
    DhtPutRecordOk { key: Vec<u8> },
    /// DHT put record failed
    DhtPutRecordErr { key: Vec<u8>, error: String },
    /// Mdns event
...
impl From<std::convert::Infallible> for PeerUPEvent {
