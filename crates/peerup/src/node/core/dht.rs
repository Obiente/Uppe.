...
impl PeerNode {
    ...
    pub fn dht_put_record(
        ...
        let record =
            Record { key: key.clone(), value, publisher: Some(self.peer_id), expires: expiration };
    ...
    pub fn dht_get_providers(&mut self, key: RecordKey) -> libp2p::kad::QueryId {
        let kademlia =
            self.swarm.behaviour_mut().kademlia.as_mut().expect("Kademlia should be enabled");
    ...
    pub fn dht_stop_providing(&mut self, key: &RecordKey) {
        if let Some(kademlia) = self.swarm.behaviour_mut().kademlia.as_mut() {
            ...
            debug!("DHT stop_providing: key={:?}", String::from_utf8_lossy(key.as_ref()));
        }
    }
}
