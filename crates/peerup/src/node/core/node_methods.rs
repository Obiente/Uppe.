...
impl PeerNode {
    ...
    pub fn add_kademlia_bootstrap_peers(&mut self, peers: &[(libp2p::PeerId, libp2p::Multiaddr)]) -> Result<()> {
    ...
    pub fn dial_bootstrap_peers(&mut self, addrs: &[String]) -> Result<()> {
        ...
        if success_count == 0 && !addrs.is_empty() {
            ...
            info!("Successfully initiated {} of {} bootstrap connections", success_count, addrs.len());
        }
